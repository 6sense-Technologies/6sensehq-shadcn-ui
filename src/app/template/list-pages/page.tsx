...
import { listPageVariations } from "@/lib/template-nav"

export const metadata: Metadata = {
  title: "List Page Templates",
  description: "Data table and list page layout variations.",
}

export default function TemplateListPagesIndexPage() {
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-8 p-6 md:p-10">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold tracking-tight">List Pages</h1>
        <p className="text-muted-foreground text-lg">
          Data-dense table and list layouts. Each opens in a new page.
        </p>
      </div>

      <div className="grid gap-2 sm:grid-cols-2">
        {listPageVariations.map((item) => (
          <Link
            key={item.slug}
            href={item.href}
            target="_blank"
