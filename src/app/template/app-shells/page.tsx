...
export const metadata: Metadata = {
  title: "App Shell Templates",
  description: "Application shell layout variations.",
}

export default function TemplateAppShellsIndexPage() {
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-8 p-6 md:p-10">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold tracking-tight">App Shells</h1>
        <p className="text-muted-foreground text-lg">
          Full application layouts with reusable sidebars. Each opens in a new
          page.
        </p>
      </div>

      <div className="grid gap-2 sm:grid-cols-2">
        {appShellVariations.map((item) => (
          <Link
            key={item.slug}
            href={item.href}
            target="_blank"
